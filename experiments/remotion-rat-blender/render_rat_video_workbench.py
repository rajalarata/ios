...
# Workbench is used only as a fast Blender-native animation QA render.
...
# Workbench reads viewport diffuse colors rather than the Principled shader.
# Mirror each Principled Base Color so the QA video preserves the character palette.
...
def kc(frame, loc, target, lens=55):
    cam.location = loc
    cam.data.lens = lens
    look_at(target)
...
# Restrained asymmetric acting for rig QA.
for f in (1, 120):
...
# Full-character establishes at both ends, with intentional medium/face coverage in the middle.
kc(1,   (0.0, -10.2, 2.25), (0, 0, 1.75), 50)
kc(30,  (5.8, -8.0, 2.55),  (0, 0, 1.82), 54)
kc(60,  (4.5, -5.4, 3.05),  (0, -0.08, 2.70), 68)
kc(90,  (-5.8, -8.0, 2.55), (0, 0, 1.82), 54)
kc(120, (0.0, -10.2, 2.25), (0, 0, 1.75), 50)
